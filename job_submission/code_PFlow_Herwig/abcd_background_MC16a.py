...
def makeHists(background,dsid,chain_name,crossection_background,label,weight_branch,signal_array):
    ...
    tagger = ['allhad_2015_3var50_btag70','allhad_2016_3var50_btag70','allhad_2015_3var50','allhad_2016_3var50','allhad_2015_3var80_btag70','allhad_2016_3var80_btag70','allhad_2015_3var80','allhad_2016_3var80']
    ...
    elif "3649" in background or "3646" in background:
        print 'Calculating dijet Herwig or Sherpa'
        for j in xrange(sumWeights.GetEntries()):
            sumWeights.GetEntry(j)
            if "364679" in background or "364680" in background:
                sumweight = getattr(sumWeights,"totalEventsWeighted")
            else:
                sumweight = getattr(sumWeights,"totalEvents")
    ...
    if "3123" in background:
        if "weight" in weight_branch:
            os.chdir("/cephfs/user/s6marahm/output_sys/output_sig_"+signal_array+"/output_"+weight_branch+"/")
        elif "CategoryReduction" in chain_name:
            os.chdir("/cephfs/user/s6marahm/output_sys/output_sig_"+signal_array+"/output_"+chain_name+"/")
        else:
            os.chdir("/cephfs/user/s6marahm/output_sys/output_sig_"+signal_array+"/output/")
    else:
        if "weight" in weight_branch:
            os.chdir("/cephfs/user/s6marahm/output_sys/output_"+weight_branch+"/")
        elif "CategoryReduction" in chain_name:
            os.chdir("/cephfs/user/s6marahm/output_sys/output_"+chain_name+"/")
        elif "crossection" in signal_array:
            os.chdir("/cephfs/user/s6marahm/output_sys/output_"+signal_array+"/")
        elif "Herwig" in signal_array:
            os.chdir("/cephfs/user/s6marahm/output_sys/output_"+signal_array+"/")
        elif "Sherpa" in signal_array:
            os.chdir("/cephfs/user/s6marahm/output_sys/output_"+signal_array+"/")
        else:
    ...
    for l in xrange(nominal.GetEntries()):
        ...
        nominal1.GetEntry(l)
        weight = 1.0
        tag2015_tight = getattr(nominal,tagger[0])
        tag2016_tight = getattr(nominal,tagger[1])
        tag2015_tight_0b = getattr(nominal,tagger[2])
        tag2016_tight_0b = getattr(nominal,tagger[3])
        tag2015_loose = getattr(nominal,tagger[4])
        tag2016_loose = getattr(nominal,tagger[5])
        tag2015_loose_0b = getattr(nominal,tagger[6])
        tag2016_loose_0b = getattr(nominal,tagger[7])
        ...
        if forward == True:
            if len(jetpt_btag) > 0 and abs(jeteta[idx_jet]) < 2.5 and DeltaR > 1.0 and met_met < 120e3 and tight[idx_ljet] == 1 and jetpt[idx_jet] > 350e3 and leadjetpt[idx_ljet] > 500e3 and (tag2015_tight == 1 or tag2016_tight == 1): #2b_tight_Wtagged
                #tight[idx_ljet] == 1, met_met < 120e3, DeltaR_bjet > 0.8
            ...
            elif len(jetpt_btag) == 0 and abs(jeteta[idx_jet]) < 2.5 and DeltaR > 1.0 and met_met < 120e3 and tight[idx_ljet] == 1 and jetpt[idx_jet] > 350e3 and leadjetpt[idx_ljet] > 500e3 and (tag2015_tight_0b == 1 or tag2016_tight_0b == 1): #0b_tight_Wtagged
            ...
            elif len(jetpt_btag) > 0 and abs(jeteta[idx_jet]) < 2.5 and DeltaR > 1.0 and met_met < 120e3 and loose[idx_ljet] == 1 and tight[idx_ljet] != 1 and jetpt[idx_jet] > 350e3 and leadjetpt[idx_ljet] > 500e3 and ((tag2015_loose == 1 and tag2015_tight != 1) or (tag2016_loose == 1 and tag2016_tight != 1)): #2b_loose_not_tight_Wtagged
            ...
            elif len(jetpt_btag) == 0 and abs(jeteta[idx_jet]) < 2.5 and DeltaR > 1.0 and met_met < 120e3 and loose[idx_ljet] == 1 and tight[idx_ljet] != 1 and jetpt[idx_jet] > 350e3 and leadjetpt[idx_ljet] > 500e3 and ((tag2015_loose_0b == 1 and tag2015_tight_0b != 1) or (tag2016_loose_0b == 1 and tag2016_tight_0b != 1)): #0b_loose_not_tight_Wtagged
                #print tight[idx_ljet],loose[idx_ljet],leadjetpt[idx_ljet],jetpt[idx_jet],len(jetpt_btag)
                ...
                #and (tag2015_loose != 1 or tag2016_loose != 1)
                ...
                #and (tag2015_loose_0b != 1 or tag2016_loose_0b != 1)
