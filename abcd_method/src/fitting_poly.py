...
fRcorr_polyfit = ROOT.TFile.Open("/cephfs/user/s6marahm/output/abcd_correlation_polyfit.root","RECREATE")
...
print "abcd_correlation_polyfit.root created!"



print "Applying polynomial fitted Rcorr to the ABCD multijet in SR.."

fnocorr = TFile.Open('/cephfs/user/s6marahm/output/abcd_output_no_corr.root',"update")
fpolyfit = TFile.Open('/cephfs/user/s6marahm/output/abcd_correlation_polyfit.root',"update")
fout = ROOT.TFile.Open("/cephfs/user/s6marahm/output/abcd_output_polyfit_SR.root","RECREATE")

for r in region_calculated:
    for v in variable_calculated:
        n2 = fnocorr.Get(v+'_'+r+'_tight_Wtagged')
        n3 = fpolyfit.Get(v+'_'+r+'_tight_Wtagged')
        temp = n2.Clone()
        temp.Multiply(n3)
        temp.Write()
fout.Close()

print "abcd_output_polyfit_SR.root created!"
